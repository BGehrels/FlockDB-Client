...
public class PagedNodeIdList implements Iterable<Long> {
	...
	private PagedNodeIdList getOtherPage(long otherPagesCursor) throws FlockException, IOException {
		...
		try {
			resultses = backingFlockClient.select2(Collections.singletonList(nextPageQuery));
		} catch (TException e) {
			throw new IOException(e);
		}
		return new PagedNodeIdList(backingFlockClient, nextPageQuery, resultses.get(0));
	}
